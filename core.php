...
class core
{
    ...
    public $seo_opt = array(
        ...
        'indexfollow'           => array('index', 'follow')
    );
    ...
    public function __construct(\phpbb\config\config $config, \phpbb\request\request $request, \phpbb\user $user, \phpbb\auth\auth $auth, $phpbb_root_path, $php_ext)
    {
        ...
        // Caching config
        @define('PHPBB_SEO_USU_ROOT_DIR', rtrim(dirname(realpath(__FILE__)), '\\/') . '/'); //@ is a dirty fix to let it work in functions_content.php
    ...
    public function add_nofollow($string)
    {
        $follow_list = array(
            'aroundtheglobe.nl',
            'aroundtheglobe.be'
